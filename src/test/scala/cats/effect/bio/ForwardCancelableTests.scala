...
import cats.effect.bio.internals.ForwardCancelable
...
class ForwardCancelableTests extends FunSuite with Matchers {
  test("cancel() after complete") {
    ...
    val ref = ForwardCancelable[Throwable]()
    ref.complete(BIO { effect += 1 })
    effect shouldBe 0
  ...
  test("cancel() before complete") {
    ...
    intercept[IllegalStateException] { ref.complete(BIO { effect += 2 }) }
